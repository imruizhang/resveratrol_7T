...
if [ -f $raw_dir/*DWI_whole_brain_tra.nii ]; then

	dti_name=$(find $raw_dir -name *DWI_whole_brain_tra.nii)
	echo $dti_name
	dti_arr=($dti_name)

	bvec_name=$(find $raw_dir -name *DWI_whole_brain_tra.bvec)
	echo $bvec_name
	bvec_arr=($bvec_name)

	bval_name=$(find $raw_dir -name *DWI_whole_brain_tra.bval)
	echo $bval_name
	bval_arr=($bval_name)


elif [ -f $raw_dir/*DWI_whole_brain_tra_A-P.nii ]; then
	dti_name=$(find $raw_dir -name *DWI_whole_brain_tra_A-P.nii)
	echo $dti_name
	dti_arr=($dti_name)

	bvec_name=$(find $raw_dir -name *DWI_whole_brain_tra_A-P.bvec)
	echo $bvec_name
	bvec_arr=($bvec_name)

	bval_name=$(find $raw_dir -name *DWI_whole_brain_tra_A-P.bval)
	echo $bval_name
	bval_arr=($bval_name)
else
	echo "file not found"
	echo "continue with next subject"
	echo "||||||||||||||||||||||||||||||||||||"
	continue
fi
